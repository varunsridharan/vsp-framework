...
if(!class_exists("VPS_Addons_File_Meta")){
    
    class VSP_Addons_File_Meta extends VSP_Class_Handler {
        ...
        public function get_metadata($addons_files ){
            ...
            foreach($addons_files as $addon_file){
                ...
                //$meta_data = $this->fix_addon_metadata($meta_data);
        ...
        private function get_plugin_meta($file,$markup = true,$translate = true){
            ...
            if(empty($data['TextDomain'])){$data['TextDomain'] = '';}
            if(empty($data['DomainPath'])){$data['DomainPath'] = false;}
            if(empty($data['category'])){$data['category'] = array('general');}
            $data['category-slug'] = sanitize_key($data['category']);
            $data['PluginURI'] = $data['addon_url'];
            
            if ( $markup || $translate ) {
                $data = _get_plugin_data_markup_translate( $file, $data, $markup, $translate );
            } 
        ...
        protected function search_addon_icon($meta){
            $name_formats = array('icon',$meta['addon_slug'],$meta['addon_slug'].'-icon','icon-'.$meta['addon_slug']);
        ...
        protected function extract_required_plugins($content,$meta){
            $reg_shortcodes = vsp_addons_extract_tags($content[5]);
            $return_array = array();
            $total_active = 0;
            if(!empty($reg_shortcodes[1])){
                foreach($reg_shortcodes[1] as $sc){
                    $data = vsp_addons_extract_tags_pattern($sc,$content[5]);
