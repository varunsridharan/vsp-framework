    ...
    class VSP_Addons_Admin extends VSP_Addons_Core {
        ...
        public function render_category_ulli_html() {
            ...
            foreach( $this->addon_cats as $slug => $name ) {
                if( $this->addons_cats_count[$slug] === 0 ) {
                    continue;
                }
        ...
        public function render_single_addon() {
            ...
            foreach( $this->addons_list as $file => $data ) {
                ...
                $addon_view_url = admin_url('plugin-install.php?&isvspaddon=true&tab=plugin-information&plugin=' . $file . '&pathid=' . $pathid . '&TB_iframe=true&width=600&height=800');
                $url            = '<a href="' . $addon_view_url . '" class="thickbox open-plugin-details-modal" title="' . esc_attr(__('View WP All Import Pro Changelog', 'vsp-framework')) . '">' . __('View details', 'vsp-framework') . '</a>';
                ...
                $html .= '<h3> <a class="thickbox open-plugin-details-modal" href="' . $addon_view_url . '" title="' . esc_attr($data['Name']) . '">' . $data['Name'] . '</a> [<small>V ' . $data['Version'] . '</small>]';
                $html .= '<a href="' . $data['icon'] . '" data-fancybox="' . $addon_slug . '-gallery" ><img src="' . $data['icon'] . '" class="plugin-icon addon-icon"/></a>';
                $html .= '</h3>';
                $html .= '</div>';

                if( ! empty($data['screenshots']) ) {
                    $html .= '<div class="hidden" style="display:none;visibility: hidden">';
                    foreach( $data['screenshots'] as $screenshot ) {
                        if( is_array($screenshot) ) {
                            $html .= '<a  data-fancybox="' . $addon_slug . '-gallery"  href="' . $screenshot['url'] . '" data-caption="' . $screenshot['title'] . '" title="' . $screenshot['title'] . '">' . $screenshot['title'] . '</a>';
                        } else {
                            $html .= '<a  data-fancybox="' . $addon_slug . '-gallery"  href="' . $screenshot . '">' . $screenshot . '</a>';
                        }
                    }
                    $html .= '</div>';
                }
                ...
                if( ! empty($_rplugins) ) {
                    ...
                    $html .= '<h3>' . __("Required Plugins", 'vsp-framework') . '</h3>';
                    $html .= $_rplugins;
                    $html .= '<p> <span>' . __("Above Mentioned Plugin name with version are Tested Upto", 'vsp-framework') . '</span> </p>';
                    $html .= '</div>';
                }
