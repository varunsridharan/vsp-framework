...
if ( ! class_exists( 'Order' ) ) :
	...
	class Order extends Data_Compatibility {
			...
			if ( \VSP\Helper\WC::is_wc_version_gte_3_0() ) {
				...
				if ( $date = $order->$prop ) {
					...
					try {
						...
						$date = new \VSP\Helper\Date_Time( $date, new \DateTimeZone( wc_timezone_string() ) );
		...
		public static function get_prop( $object, $prop, $context = 'edit', $compat_props = array() ) {
			...
			if ( \VSP\Helper\WC::is_wc_version_lt_3_0() ) {
		...
		public static function get_item_formatted_meta_data( $item, $hideprefix = '_', $include_all = false ) {
			...
			if ( \VSP\Helper\WC::is_wc_version_gte_3_1() && $item instanceof \WC_Order_Item ) {
		...
		public static function get_edit_order_url( \WC_Order $order ) {
			...
			if ( \VSP\Helper\WC::is_wc_version_gte( '3.3' ) ) {
