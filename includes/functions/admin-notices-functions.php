...
if ( ! function_exists( 'vsp_notices' ) ) {
	 ...
	 * @param string $type .
	 *
	 * @return \VSP\Modules\WordPress\Admin\Notices|\VSP\Modules\WordPress\WP_Notice
	 */
	function vsp_notices( $type = '' ) {
		if ( empty( $type ) ) {
			...
			if ( ! isset( $vsp_notices ) ) {
				$vsp_notices = \VSP\Modules\WordPress\Admin\Notices::getInstance();
			}
			return $vsp_notices;
		}

		$_instance = new \VSP\Modules\WordPress\WP_Notice();

		switch ( $type ) {
			case 'error':
				$_instance->setType( \VSP\Modules\WordPress\WP_Notice::TYPE_ERROR );
				break;
			case 'update':
				$_instance->setType( \VSP\Modules\WordPress\WP_Notice::TYPE_UPDATED );
				break;
			case 'upgrade':
				$_instance->setType( \VSP\Modules\WordPress\WP_Notice::TYPE_UPDATED_NAG );
				break;
		}
		return $_instance;
	}
...
if ( ! function_exists( 'vsp_notice_error' ) ) {
	 ...
	 * @param array $args
	 */
	function vsp_notice_error( $title, $message, $times = 1, $screen = array(), $args = array() ) {
		$args['title']  = $title;
		$args['times']  = $times;
		$args['screen'] = $screen;
...
if ( ! function_exists( 'vsp_js_alert' ) ) {
	...
	function vsp_js_alert( $title = '', $text = '', $type = '', $options = array(), $queue = '' ) {
		...
		$opts        = \VSP\Modules\Localizer::handle_js_function( $opts );

		$return_html .= \VSP\Helper::php_to_js( $name, $opts, false );
